...
const getProducts = async (req: NextApiRequest, res: NextApiResponse) => {
    try{
        const { category, sortby, size, minPrice, maxPrice, lang } = req.query;

        const categoryFilter = category && category !== 'null' ? { categoryid: category } : {};
        ...
        const products = await Products.find({ ...categoryFilter, ...sizeFilter, ...priceFilter }).sort({...sortbyFilter } as {})
