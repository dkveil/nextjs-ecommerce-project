...
export const SizeSubform = ({ title, inStock, handleSize, closeSizeSubform, availableSizes }: ISizeSubform) => {
    ...
    const handleClick = () => {
        if (sizeNameRef.current && sizeInStockRef.current) {
            ...
            if (sizeInStock.toString().length === 0 || Number.isNaN(sizeInStockRef.current.value) || sizeInStock < 0) {
    ...
    React.useEffect(() => {
        if (sizeNameRef.current && title && sizeInStockRef.current && inStock) {
...
const CreateProductPage = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        engDescription: Yup.string().required(texts[currentLanguage].engdescrequired),
        polDescription: Yup.string().required(texts[currentLanguage].poldescrequired),
    ...
    const formik = useFormik<IFormModel>({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const imagesUploaded = values.images.filter((image) => (image as IImage).url);
                const imagesToUpload = values.images.filter((image) => !(image as IImage).url);

                const newImages = await imageUpload(imagesToUpload as File[]);

                const images = [...imagesUploaded, ...newImages];
                ...
                const { messageid } = await postData('products', productData, user?.accessToken);
    ...
    const handleSize = ({ title, inStock }: { title: string; inStock: number }) => {
        if (values.options.find((option) => option.title === title)) {
            const newOptionState = values.options.map((option) => {
            ...
            <ManagerSection>
                <div className="container">
                    <div className="manager__header">
                        <h2>{texts[currentLanguage].createproduct}</h2>
                    </div>
